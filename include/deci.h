...
class Decimation
{
    ...
    lit* lunit_clause_queue;
    int* sense_in_lunit_clause_queue;
    int lunit_beg_pointer;
    int lunit_end_pointer;
...
void Decimation::make_space(int max_c, int max_v)
{
    ...
    lunit_clause_queue=new lit[max_v];
    sense_in_lunit_clause_queue=new int[max_v];
...
void Decimation::free_memory()
{
    ...
    delete[] lunit_clause_queue;
    delete[] sense_in_lunit_clause_queue;
...
void Decimation::init(int* ls_local_opt, int* ls_global_opt, lit* ls_unit_clause, int ls_unit_clause_count, int* ls_clause_lit_count)
{
    ...
    for(int i=0;i<num_vars;++i)
    {
        ...
        sense_in_lunit_clause_queue[v]=-1;// not in lunit queue
    }
...
void Decimation::push_unit_clause_to_queue(lit tem_l)
{
    ...
    else
    {
        ...
        if(sense_in_lunit_clause_queue[v]==-1)
        {
            sense_in_lunit_clause_queue[v]=tem_l.sense;
            lunit_clause_queue[lunit_end_pointer++]=tem_l;
        }
        else
        {
            if(sense_in_lunit_clause_queue[v]!=tem_l.sense)//conflict var in hard unit queue
            {
                sense_in_lunit_clause_queue[v]=-3;//means this variable is conflict in hard unit queue
            }
        }
    }
}
...
void Decimation::lunit_propagation()
{
    int v,c,sense;
    v=lunit_clause_queue[lunit_beg_pointer].var_num;
    sense=lunit_clause_queue[lunit_beg_pointer].sense;
    lunit_beg_pointer++;

    if(fix[v]!=-1) return ;

    if(sense_in_lunit_clause_queue[v]==-3)
    {
        sense=choose_sense(v);
    }
    assign(v,sense);
...
void Decimation::unit_prosess()
{
    while(unassigned_var_count>0)
    {
        ...
        else if(lunit_beg_pointer!=lunit_end_pointer)
        {
            lunit_propagation();
        }
        else
